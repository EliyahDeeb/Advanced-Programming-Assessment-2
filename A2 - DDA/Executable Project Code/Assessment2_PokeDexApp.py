...
# Colors used to distinguish each Pokémon type.
type_colors = {
    ...
    "steel": "#B8B8D0", "fighting": "#C03028",
}


# Function to generate a gradient for the Pokémon’s types.
...
def fetch_pokemon_data():
    ...
    try:
        ...
        background_label.image = gradient_image  # Keep a reference
...
# Function to fetch and display the evolution chain of a Pokémon.
def fetch_evolution_chain(species_url):
    try:
        ...
        def traverse_chain(chain, current_chain=[]):
            ...
            if 'evolves_to' in chain and chain['evolves_to']:
                ...
                for next_chain in chain['evolves_to']:
                    traverse_chain(next_chain, current_chain[:])  # Pass a copy
            else:
                # When there's no further evolution, append this path
        ...
        if evolution_chain:
            evolution_label.config(text=f"Evolution: {' // '.join(evolution_chain)}",
                                 font=("Pixelify Sans", 12), bg=None)
        else:
            evolution_label.config(text="Evolution: No evolution found.",
                                 font=("Pixelify Sans", 12), bg=None)
...
# Background label for the gradient.
background_label = tk.Label(window)
background_label.place(relwidth=1, relheight=1)

# Frame for the PokéDex’s search section.
search_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
...
search_button = tk.Button(search_frame, text="Search", command=fetch_pokemon_data,
                          font=("Pixelify Sans", 12))
search_button.grid(row=0, column=2, padx=10, pady=5)

# Sprite frame
sprite_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
...
# Info frame
info_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
...
# Stats frame
stats_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
...
# Abilities frame
abilities_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
...
# Evolution frame
evolution_frame = tk.Frame(window, bd=2, relief="solid", padx=10, pady=10)
